...
export default function TravelTricks() {
  const [activeTab, setActiveTab] = useState('gear'); // 'gear' | 'prep' | 'packing' | 'onTheRoad' | 'burnout' | 'all'
  const [gearCategory, setGearCategory] = useState('all');
  const [searchQuery, setSearchQuery] = useState('');
  const [fanosOpen, setFanosOpen] = useState(false);

  const gearItems = [
    { name: 'DJI Neo / Mini Drone', desc: 'Ultralight aerial travel drone', category: 'Tech', link: AFFILIATE_LINKS.dji_drone, icon: '🚁' },
  ...
  const tabs = [
    { id: 'gear', label: '🎒 Our Gear Stack', badge: '21 items' },
    { id: 'prep', label: '📝 Preparation', badge: '5 tips' },
    { id: 'packing', label: '🧳 Packing Rules', badge: '8 tips' },
    { id: 'onTheRoad', label: '🌏 On The Road', badge: '10 tips' },
    { id: 'burnout', label: '🧘 Avoiding Burnout', badge: 'Rituals & Mindset' },
    { id: 'demo', label: '📱 Live App Demo', badge: 'Interactive' },
  ];
      ...
      <div className="container" style={{ maxWidth: '980px', margin: '0 auto' }}>
        ...
        <header style={{ textAlign: 'center', marginBottom: '28px' }}>
          <span style={{
            ...
            Tested Travel Blueprint
          </span>
          <h1 style={{ fontSize: '3.2rem', color: 'var(--color-purple)', fontFamily: 'var(--font-heading)', margin: '0 0 10px', lineHeight: 1.05 }}>
            Travel Tricks & Wisdom Hub
          </h1>
          <p style={{ fontSize: '1.1rem', color: '#555', maxWidth: '650px', margin: '0 auto', lineHeight: 1.5 }}>
            Everything we’ve learned from full-time travel — packing systems, booking hacks, and relationship rituals. Select a section below to explore!
          </p>
        </header>

        {/* FUN ORGANIC TAB SWITCHER BAR */}
        <div style={{
          display: 'flex',
          gap: '8px',
            ...
            return (
              <button
                ...
                style={{
                  ...
                  boxShadow: isActive ? '0 4px 12px rgba(133, 58, 81, 0.2)' : 'none'
                }}
              >
                <span>{tab.label}</span>
                <span style={{
          ...
          <button
            ...
            style={{
              ...
              borderRadius: '20px',
              border: 'none',
              backgroundColor: activeTab === 'all' ? 'var(--color-golden)' : 'transparent',
              color: activeTab === 'all' ? 'var(--color-purple)' : '#888',
              fontWeight: 800,
              fontSize: '0.85rem',
              cursor: 'pointer'
            }}
          >
            {activeTab === 'all' ? '✦ Showing All' : 'Show All Sections'}
          </button>
        </div>

        {/* TAB CONTENT HUB CANVAS */}
        <div style={{
          backgroundColor: 'white',
          borderRadius: '28px',
          padding: '28px 24px',
          ...
          {/* TAB 1: OUR GEAR STACK */}
          {(activeTab === 'gear' || activeTab === 'all') && (
            <div style={{ marginBottom: activeTab === 'all' ? '40px' : 0 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px', marginBottom: '16px' }}>
                <div>
                  <span style={{ color: 'var(--color-golden)', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '1px', fontSize: '0.72rem' }}>Field-Tested</span>
                  <h2 style={{ fontSize: '1.8rem', color: 'var(--color-purple)', fontFamily: 'var(--font-heading)', margin: 0 }}>
                    🎒 Our Tested Travel Gear Stack
                  </h2>
                </div>
                ...
                {gearCategories.map((cat) => (
                  <button
                    ...
                    style={{
                      ...
                      borderRadius: '14px',
                      border: 'none',
                      backgroundColor: gearCategory === cat ? 'var(--color-purple)' : '#F3F4F6',
              ...
              {/* HIGH-DENSITY HIGHLY EFFICIENT GEAR GRID */}
              <div style={{
                display: 'grid',
                ...
                {filteredGear.map((item, idx) => (
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: '0.85rem', fontWeight: 700, color: 'var(--color-purple)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                        {item.name}
                      </div>
          ...
          {/* TAB 2: PREPARATION */}
          {(activeTab === 'prep' || activeTab === 'all') && (
            <div style={{ marginBottom: activeTab === 'all' ? '40px' : 0 }}>
              <span style={{ color: 'var(--color-orange)', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '1px', fontSize: '0.72rem' }}>Pre-Trip Systems</span>
              <h2 style={{ fontSize: '1.8rem', color: 'var(--color-purple)', fontFamily: 'var(--font-heading)', margin: '0 0 16px' }}>
          ...
          {/* TAB 3: PACKING RULES */}
          {(activeTab === 'packing' || activeTab === 'all') && (
            <div style={{ marginBottom: activeTab === 'all' ? '40px' : 0 }}>
              <span style={{ color: 'var(--color-golden)', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '1px', fontSize: '0.72rem' }}>Bag Setup</span>
              <h2 style={{ fontSize: '1.8rem', color: 'var(--color-purple)', fontFamily: 'var(--font-heading)', margin: '0 0 16px' }}>
                🧳 2. Lightweight Packing Rules
              </h2>
              ...
              {/* Harry's Soccer Ball Organic Post-it Card */}
          ...
          {/* TAB 4: ON THE ROAD */}
          {(activeTab === 'onTheRoad' || activeTab === 'all') && (
            <div style={{ marginBottom: activeTab === 'all' ? '40px' : 0 }}>
              <span style={{ color: 'var(--color-purple)', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '1px', fontSize: '0.72rem' }}>Smart Travel</span>
              <h2 style={{ fontSize: '1.8rem', color: 'var(--color-purple)', fontFamily: 'var(--font-heading)', margin: '0 0 16px' }}>
                🌏 3. On The Road Secrets
              </h2>
          ...
          {/* TAB 5: AVOIDING BURNOUT */}
          {(activeTab === 'burnout' || activeTab === 'all') && (
            <div style={{ marginBottom: activeTab === 'all' ? '40px' : 0 }}>
              <span style={{ color: 'var(--color-orange)', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '1px', fontSize: '0.72rem' }}>Mindset & Harmony</span>
              <h2 style={{ fontSize: '1.8rem', color: 'var(--color-purple)', fontFamily: 'var(--font-heading)', margin: '0 0 16px' }}>
                🧘 4. Avoiding Burnout & Relationship Rituals
              </h2>
              ...
              <div style={{ marginBottom: '16px' }}>
                <strong style={{ fontSize: '1rem', color: 'var(--color-purple)', display: 'block', marginBottom: '10px' }}>☀️ Our 3 Daily Morning Affirmations:</strong>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '10px' }}>
                  <div style={{ backgroundColor: '#FFFDF5', padding: '12px 14px', borderRadius: '12px', border: '1px solid #FCD34D', fontFamily: 'var(--font-hand)', fontSize: '1.1rem', color: 'var(--color-purple)' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '8px', fontSize: '0.85rem', color: '#555' }}>
                ...
                <div style={{ padding: '10px 12px', backgroundColor: '#FAFAFA', borderRadius: '10px' }}><strong>😂 Laugh:</strong> Laugh when you want to cry.</div>
                <div style={{ padding: '10px 12px', backgroundColor: '#FAFAFA', borderRadius: '10px' }}><strong>🙏 Gratitude:</strong> Travel is a privilege.</div>
              </div>
            </div>
          )}

          {/* TAB 6: LIVE APP SIMULATOR */}
